...
public class KolodaView: UIView, DraggableCardDelegate {
    ...
    private func subscribeForNotifications() {
        NotificationCenter.default.addObserver(self, selector: "layoutDeck", name: UIDevice.orientationDidChangeNotification, object: nil)
    }
        ...
        if let dataSource = dataSource {
            ...
            if countOfCards - currentCardNumber > 0 {
                ...
                for index in 0..<countOfNeededCards {
                    let nextCardContentView = dataSource.koloda(koloda: self, viewForCardAtIndex: UInt(index+currentCardNumber))
                    ...
                    let overlayView = overlayViewForCardAtIndex(index: UInt(index+currentCardNumber))
                    
                    nextCardView.configure(view: nextCardContentView, overlayView: overlayView)
                ...
                self.delegate?.koloda(koloda: self, didSelectCardAtIndex: UInt(currentCardNumber))
            }
        }
    }
        ...
        if !visibleCards.isEmpty {
            for (index, card) in visibleCards.enumerated() {
                if index == 0 {
                    ...
                    let cardParameters = backgroundCardParametersForFrame(initialFrame: frameForCardAtIndex(index: UInt(index)))
    ...
    public func frameForCardAtIndex(index: UInt) -> CGRect {
        ...
        let previousCardFrame = index > 0 ? frameForCardAtIndex(index: max(index - 1, 0)) : CGRect.zero
    ...
    private func frameForTopCard() -> CGRect {
        return frameForCardAtIndex(index: 0)
    }
    ...
    private func moveOtherCardsWithFinishPercent(percent: CGFloat) {
        if visibleCards.count > 1 {
            ...
            for index in 1..<visibleCards.count {
                let previousCardFrame = frameForCardAtIndex(index: UInt(index - 1))
                var frame = frameForCardAtIndex(index: UInt(index))
                ...
                let cardParameters = backgroundCardParametersForFrame(initialFrame: frame)
                let scale = cardParameters.scale
    ...
    func card(card: DraggableCardView, wasDraggedWithFinishPercent percent: CGFloat, inDirection direction: SwipeResultDirection) {
        ...
        if let shouldMove = delegate?.koloda(kolodaShouldMoveBackgroundCard: self), shouldMove == true {
            self.moveOtherCardsWithFinishPercent(percent: percent)
        }
        delegate?.koloda(koloda: self, draggedCardWithFinishPercent: percent, inDirection: direction)
    }
    
    func card(card: DraggableCardView, wasSwipedInDirection direction: SwipeResultDirection) {
        swipedAction(direction: direction)
    }
    ...
    func card(cardWasReset card: DraggableCardView) {
        if visibleCards.count > 1 {
            ...
            UIView.animate(withDuration: backgroundCardFrameAnimationDuration,
                ...
                options: .curveLinear,
                animations: {
                    self.moveOtherCardsWithFinishPercent(percent: 0)
                },
                completion: {
                    _ in
                    self.animating = false
    ...
    func card(cardWasTapped card: DraggableCardView) {
        ...
        delegate?.koloda(koloda: self, didSelectCardAtIndex: UInt(index))
    }
    ...
    private func overlayViewForCardAtIndex(index: UInt) -> OverlayView? {
        return dataSource?.koloda(koloda: self, viewForCardOverlayAtIndex: index)
    }
    ...
    private func swipedAction(direction: SwipeResultDirection) {
        ...
        if shownCardsCount - 1 < countOfCards {
            ...
            if let dataSource = dataSource {
                ...
                let lastCardContentView = dataSource.koloda(koloda: self, viewForCardAtIndex: UInt(shownCardsCount - 1))
                let lastCardOverlayView = dataSource.koloda(koloda: self, viewForCardOverlayAtIndex: UInt(shownCardsCount - 1))
                
                let lastCardFrame = frameForCardAtIndex(index: UInt(visibleCards.count))
                let cardParameters = backgroundCardParametersForFrame(initialFrame: lastCardFrame)
                ...
                lastCardView.configure(view: lastCardContentView, overlayView: lastCardOverlayView)
        ...
        if !visibleCards.isEmpty {
            ...
            for (index, currentCard) in visibleCards.enumerated() {
                ...
                if index != 0 {
                    ...
                    frameAnimation.completionBlock = {(animation, finished) in
                        ...
                        self.delegate?.koloda(koloda: self, didSwipedCardAtIndex: UInt(self.currentCardNumber - 1), inDirection: direction)
                        self.delegate?.koloda(koloda: self, didShowCardAtIndex: UInt(self.currentCardNumber))
                        if self.shouldTransparentize {
            ...
            delegate?.koloda(koloda: self, didSwipedCardAtIndex: UInt(currentCardNumber - 1), inDirection: direction)
            animating = false
    ...
    public func revertAction() {
        if currentCardNumber > 0 && animating == false {
            ...
            if let dataSource = self.dataSource {
                let firstCardContentView = dataSource.koloda(koloda: self, viewForCardAtIndex: UInt(currentCardNumber))
                let firstCardOverlayView = dataSource.koloda(koloda: self, viewForCardOverlayAtIndex: UInt(currentCardNumber))
                ...
                firstCardView.configure(view: firstCardContentView, overlayView: firstCardOverlayView)
                ...
                applyRevertAnimation(card: firstCardView, complete: {
                    self.delegate?.koloda(koloda: self, didShowCardAtIndex: UInt(self.currentCardNumber))
                })
            }
    ...
    private func loadMissingCards(missingCardsCount: Int) {
        if missingCardsCount > 0 {
            ...
            for index in startIndex...endIndex {
                let nextCardView = DraggableCardView(frame: frameForCardAtIndex(index: UInt(index)))
    ...
    private func reconfigureCards() {
        for index in 0..<visibleCards.count {
            if let dataSource = self.dataSource {
                ...
                let currentCardContentView = dataSource.koloda(koloda: self, viewForCardAtIndex: UInt(currentCardNumber + index))
                let overlayView = dataSource.koloda(koloda: self, viewForCardOverlayAtIndex: UInt(currentCardNumber + index))
                let currentCard = visibleCards[index]
                
                currentCard.configure(view: currentCardContentView, overlayView: overlayView)
            }
        }
    }
    ...
    public func reloadData() {
        ...
        if countOfCards - (currentCardNumber + visibleCards.count) > 0 {
            ...
            if !visibleCards.isEmpty {
                loadMissingCards(missingCardsCount: missingCards)
            } else {
                setupDeck()
                layoutDeck()
    ...
    public func swipe(direction: SwipeResultDirection) {
        if (animating == false) {
            ...
            if let frontCard = visibleCards.first {
                ...
                frontCard.swipe(direction: direction)
            }
        }
    }
